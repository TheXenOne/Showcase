...
public:
	...
	void SetDigraph(bool digraph) { m_bDigraph = digraph; }
...
private:
	...
	//Vector containing removed edges
	EdgeListVector m_RemovedEdges;
...
public:
	...
	private:
		...
		SparseGraph<node_type, edge_type>&  graph;
	...
	public:
			...
			NodeIndex(node)
		{
			...
			*/
			...
			curEdge = graph.m_Edges[NodeIndex].begin();
		}
		...
		bool end()
		{
			return (curEdge == graph.m_Edges[NodeIndex].end());
		}
	};
...
void SparseGraph<node_type, edge_type>::RemoveNode(int node)
{
	...
	//EdgeType edge = EdgeType(node, node, 1.f);
	//AddEdge(edge);
...
bool SparseGraph<node_type, edge_type>::Load(std::ifstream& stream)
{
	...
	for (int e = 0; e<NumEdges; ++e)
	{
		...
		EdgeType NextEdge(from, to, cost);

		if (from != invalid_node_index && to != invalid_node_index)
		{
...
template<class node_type, class edge_type>
bool SparseGraph<node_type, edge_type>::Load(std::vector<std::string> data)
{
	Clear();

	int NumNodes, NumEdges;

	NumNodes = std::stoi(data[0]);

	for (int n = 1; n < NumNodes + 1; ++n)
	{
		std::vector<std::string> line = SplitString(data[n]);

		int nodeIndex = std::stoi(line[1]);
		int posX = std::stoi(line[3]);
		int posY = std::stoi(line[5]);

		NodeType NewNode(nodeIndex);
		NewNode.SetPosition(Vector2(posX, posY));
	...
	//now add the edges
	NumEdges = std::stoi(data[1 + NumNodes]);

	for (int e = 2 + NumNodes; e < 1 + NumNodes + NumEdges; ++e)
	{
		std::vector<std::string> line = SplitString(data[e]);
		int from = std::stoi(line[1]);
		int to = std::stoi(line[3]);
		float cost = std::stof(line[5]);
...
template<class node_type, class edge_type>
std::vector<std::string> SparseGraph<node_type, edge_type>::SplitString(const std::string & string)
{
